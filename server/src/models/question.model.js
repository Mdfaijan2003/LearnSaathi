...
questionSchema.pre("validate", function (next) {
  ...
  if (this.type === "mcq") {
    ...
    if (this.options.length > 6) {
      return next(new Error("Maximum 6 options allowed"));
    }

    let correct = 0;

    const texts = this.options.map(o => o.text.trim().toLowerCase());

    if ( new Set(texts).size !== texts.length) {
      return next(new Error( "Duplicate options not allowed"));
    }
  ...
  if (this.type === "saq") {
    ...
    this.correctAnswer = String(this.correctAnswer).trim().toLocaleLowerCase();
  }
